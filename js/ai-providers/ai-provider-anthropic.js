...
async function anthropicCompleteText({ apiKey, model, prompt, enableWebSearch }) {
  if (!apiKey) {
  ...
  if (!response.ok) {
    ...
    throw new AIProviderError(`Anthropic API antwortete mit Status ${response.status}${detail ? ': ' + detail : ''}.`);
  }
...
const AnthropicProvider = {
  ...
  async fetchEstimate({ apiKey, model, country, city, departureAirport, travelDateRange, enableWebSearch }) {
    ...
    const text = await anthropicCompleteText({ apiKey, model, prompt, enableWebSearch });
